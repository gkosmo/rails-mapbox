class MushroomsController < ApplicationController
  ...
  # GET /mushrooms.json
  def index
    @mushrooms = Mushroom.where.not(latitude: nil, longitude: nil)
    @markers = @mushrooms.map do |shroom|
      {
        lng: shroom.longitude,
        lat: shroom.latitude
      }
    end
  ...
  private
    ...
    def mushroom_params
      params.require(:mushroom).permit(:name, :address, :country)
    end
end
